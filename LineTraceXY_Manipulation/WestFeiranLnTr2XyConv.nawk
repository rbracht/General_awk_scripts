BEGIN{
	...
	#          
	#       
	# Defining three points of the survey in terms of traces and
        # lines or columns and rows. Again, these can be any 3 arbitrary 
	# non-colinear points.
	...
	# Minimum trace number (minimum column number/xline number)
	MinTrace = 2200

	# Maximum trace number (maximum column number/xline number)
	MaxTrace = 2200

	# Trace increment (column number increment)
	TraceInc = 50

	# Minimum line number (minimum row number/inline number)
	MinLine = 1079

	# Maximum line number (maximum row number/inline number)
	MaxLine = 1079
